...
export function MessageContent({ content, role }: MessageContentProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {thinkContent && (
        <details
          className="group rounded-lg border border-amber-500/30 bg-amber-500/5"
          open={isThinkExpanded}
          onToggle={(e) => setIsThinkExpanded(e.currentTarget.open)}
        >
          <summary className="cursor-pointer select-none px-4 py-2 text-sm font-medium text-amber-400 transition-colors hover:text-amber-300">
          ...
          <div className="border-t border-amber-500/20 px-4 py-3 text-sm leading-relaxed text-amber-100/80">
            <p className="whitespace-pre-wrap italic">{thinkContent}</p>
          </div>
        </details>
      )}
